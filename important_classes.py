...
class Inventory:
    ...
    def __call__(self):
        print(f"┌{'─' * 70}┐")
        print(f"│{f'MEGA COOL INVETORY TYPE SHI~':^{70}}│")
        print(f"└{'─' * 70}┘")
        if not(self.inv):
            print(f"{f"│Empty... you're cooked by now":<{70}}│")
...
class Interface:
    ...
    def get_lines(self):
        ...
        lines.append(f"├{'─' * (self.width - 2)}┤")
        lines.append(f"│{"Журнал событий":^{self.width - 2}}│")
        lines.append(f"├{'─' * (self.width - 2)}┤")
        ...
        for i, j in enumerate(inventory.inv):
            if i == inventory.order:
                ...
                lines.append(f"│{f'\033[7;92m{j}: {inventory.inv[j][1]}\033[0m':<{self.width + 9}}│")
            else:
                lines.append(f"│{f'{j}: {inventory.inv[j][1]}':<{self.width - 2}}│")
        lines.append(f"│{f'x - destroy item':<{self.width - 2}}│")
        lines.append(f"│{f'e - use item':<{self.width - 2}}│")
        lines.append(f"│{f'i - for inventory':<{self.width - 2}}│")
        lines.append(f"│{f'esc - to close':<{self.width - 2}}│")
        lines.append(f"└{'─' * (self.width - 2)}┘")
